        ...
        try:
            ...
            for device in devices:
                ...
                for entry in firewallEntries:
                    trafficRule = {'type': 'None', 'action': entry['action'], 'source': device['ip'],
                                   'destination': 'None'}
            ...
            if os.path.isfile('/opt/sweetsecurity/client/iptables/iptables_existing.sh'):
                ...
                existingHash = hashlib.md5(
                    open('/opt/sweetsecurity/client/iptables/iptables_existing.sh', 'rb').read()).hexdigest()
                newHash = hashlib.md5(
                    open('/opt/sweetsecurity/client/iptables/iptables_new.sh', 'rb').read()).hexdigest()
                if newHash != existingHash:
                    os.remove('/opt/sweetsecurity/client/iptables/iptables_existing.sh')
                    shutil.move('/opt/sweetsecurity/client/iptables/iptables_new.sh',
                                '/opt/sweetsecurity/client/iptables/iptables_existing.sh')
